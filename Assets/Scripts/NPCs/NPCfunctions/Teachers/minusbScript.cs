...
public class minusbScript : NPC
{
    ...
    {
        base.OnStart();
        audio = GetComponent<AudioManagerLiveReaction>();
        GetAngry(0f);
        Move();
        if (endless) Endless();
        Wander();
    ...
    {
        base.OnEnable();
        gc.minusbee.Add(this);
        Move();
    }
    public override void OnDisable()
    {
        base.OnDisable();
        gc.minusbee.Remove(this);
    }
    ...
    public override void OnUpdate()
    {
        ...
        targe();
        base.OnUpdate();
        base.agentSpeed = base.DefaultAgentSpeed * (ForceNoSpeed ? 0 : base.agentSpeedScale);
    ...
    public void Move()
    {
        if (isActiveAndEnabled)
        {
            ...
            if (!Muted) audio.PlaySingleClip(minusBeeSfx);
            resetWaitTime();
        }
    }
    public void resetWaitTime()
    {
        Wait = (-3 - TempAnger) * Anger / (Anger / SpeedScale) + 3;
    }
    ...
    private void OnTriggerStay(Collider play)
    {
        if (play.CompareTag("Player") & !gc.debugMode & !gc.player.titlecard)
        {
			...
			{
				gc.player.SetHP(PlayerScript.HealthChangeMode.Remove, 25 / gc.player.PlayerDmgResistance, 0.75f, false, true, false);
    ...
    public void GetAngry(float value)
    {
        Anger += value;
        SpeedScale += value/4;
        GetTempAngry(value/16);
        base.DefaultAgentSpeed += value*2;
    ...
    public bool stopMoving, antiHearing, Muted, ForceNoSpeed;
    ...
    public AudioObjectyeah minusBeeSfx;
    private AudioManagerLiveReaction audio;
    #endregion
}