...
public class scoreSystemManager : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (!stopzoombindown) zoomscorval -= Time.deltaTime; 
        if (zoomscorval < 1f)
		{
            zoomscorval = 1f;
            stopzoombindown = true;
		}
        if (!stopzoomRank) zoomRankval -= Time.deltaTime; 
        if (zoomRankval < 1f)
		{
            zoomRankval = 1f;
            stopzoomRank = true;
    ...
    public void Detectrank()
    {
        ...
        //{
            for (int i = 0; i < Ranks.Length; ++i)
            {
                if (Ranks[i].rankScore <= Mathf.Round(curScore))
                {
                    ...
                    {
                        RankAudioSource.PlaySingleClip(Ranks[i].rankSound);
        ...
        //}
        if (!stopUpdatingTSDiscord) GameControllerScript.Instance.discordupdate(discordUpdateType);
        
    }
    public void AddScore(int score,bool AffectedByMultipler = true,bool UpdateDiscord = false, string UpdateType = "chees")
    {
        if (!AffectedByMultipler) realscore = score;
        if (AffectedByMultipler) realscore = score*PointsMultiplier;
        scorevalue += realscore;
        if (UpdateDiscord) discordUpdateType = UpdateType;
        zoomscorval = 1.2f;
	...
	public class RanksShit
    {
        ...
        public AudioObjectyeah rankSound;
	}
    ...
    public AudioManagerLiveReaction RankAudioSource;
}
